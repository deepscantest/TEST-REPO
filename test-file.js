...
var DEFECT_STATUS_NEW_LABEL = 'New';
var DEFECT_STATUS_TRIAGED_LABEL = 'Triaged';
                ...
                if (parsedValue && parsedValue.length > 0) {
                    if (key === 'status') {
                        parsedValue = parsedValue.reduce(function (statuses, status) {
                            if (status === DEFECT_STATUS_NEW_LABEL) {
                                statuses.push(constants.LITE_DEFECT_STATUS_NEW);
                            } else if (status === DEFECT_STATUS_TRIAGED_LABEL) {
                                statuses.push(constants.LITE_DEFECT_STATUS_TRIAGED);
...
mainRouter['post']('/api/projects/:projectId/webhook', authMgr.ensureLogin, function (req, res, next) {
    var pid = req.params.projectId;

    req.resultPromise = jsaManager.addWebhook(pid, req.session.githubAccessToken);
    return next();
});

// remove webhook from repository
mainRouter['delete']('/api/projects/:projectId/webhook', authMgr.ensureLogin, function (req, res, next) {
    var pid = req.params.projectId;

    req.resultPromise = jsaManager.removeWebhook(pid, req.session.githubAccessToken);
    return next();
});
...
mainRouter['get']('/api/users', authMgr.ensureLogin, function (req, res, next) {
...
mainRouter['post']('/api/webhook/github', function (req, res, next) {
    ...
    if (headers['x-hub-signature']) {
        ...
        if (compare(headers['x-hub-signature'], signature)) {
            ...
            }else if (headers['x-github-event'] === 'ping') {
                //ping event : webhook 생성/삭제시 발생
                logger.debug('github ping event arrived!');
                req.resultPromise = Promise.resolve();
            } else {
...
// Get grade.svg file - for resource request
mainRouter['get']('/api/projects/:projectId/branches/:branchId/badge/grade.svg', function (req, res, next) {
    var branchId = req.params.branchId;
    jsaManager.getBadgePath(branchId)
        .then(function (path) {
            res.sendFile(path, function (err) {
                if (err) {
                    ...
                    logger.debug('File sent: ', path);
                }
            });
    }).catch(function (err) {
        if (err instanceof JsaError) {
            logger.error(err.message, err.statusCode, err.stack);
...
// Get badges - for jsa-client
mainRouter['get']('/api/projects/:projectId/branches/:branchId/badge', function (req, res, next) {
    var pid = req.params.projectId;
    var bid = req.params.branchId;
    req.resultPromise = jsaManager.getBadgeData(pid, bid);
